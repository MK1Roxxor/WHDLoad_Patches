...
; 14-May-2021	- problem with copperlist 2 fixed
;		- black screen after first title picture has been shown
;		  and no key was pressed fixed

; 26-Nov-2018	- work started, adapted from my Decimal Dungeon patch
...
slv_CurrentDir	IFD	DEBUG
		dc.b	"SOURCES:WHD_Slaves/FractionAction/"
		ENDC
		dc.b	"data",0

slv_name	dc.b	"Fraction Action",0
slv_copy	dc.b	"1986 Unicorn Software",0
slv_info	dc.b	"installed by StingRay/[S]carab^Scoopex",10
		IFD	DEBUG
		dc.b	"DEBUG!!! "
		ENDC
		dc.b	"Version 1.01 (14.05.2021)",0
slv_config	dc.b	0
...
.run	lea	PT_GAME(pc),a1
	...
	; store loaded segment list in current task as
	; the game is auto-detaching
	move.l	a6,-(a7)
	sub.l	a1,a1
	move.l	$4.w,a6
	jsr	_LVOFindTask(a6)
...
PT_GAME	dc.l	$29f8,$2acf,$4528,PLGAME-PT_GAME	; 
	dc.l	-1				; end of tab

PLGAME	PL_START
	PL_SA	$0,$4f0			; skip HLS decryption
...
.FixCop	move.l	a6,-(a7)
	move.l	$4.w,a6
	move.l	$9c(a6),a6		; a6: Gfxbase

	; fix problem with illegal Copperlist 2
	move.l	gb_copinit(a6),gb_LOFlist(a6)

	; WaitTOF fixes the problem with the black screen when no key
	; has been pressed after the first title picture fades down
	jsr	_LVOWaitTOF(a6)
	move.l	(a7)+,a6
...
; Fraction Action decrypter
; stingray, 26-Nov-2018
...
RELOC	DC.L	$00060008,$00160006,$001E0008,$0031000B
	DC.L	$0043000B,$00530015,$00700005,$00770006
	DC.L	$007F000E,$008F0007,$009B0006,$00B00009
	DC.L	$00C1001A,$00000000
